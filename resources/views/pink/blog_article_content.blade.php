    ...
    <div id="content-single" class="content group">
        <div class="hentry hentry-post blog-big group ">
            ...
            <div class="meta group">
                ...
                <p class="comments"><span><a href="#comments" title="Comment on {{ $article->title }}.">{{ count($article->comments)>0  ? count($article->comments) : '0' }} {{ Lang::choice('blog.comments', count($article->comments))}}</a></span></p>
            </div>
            <!-- post content -->
        ...
        <div id="comments">
            <h3 id="comments-title">
                <span>{{ count($article->comments)>0  ? count($article->comments) : '0' }}</span> {{ Lang::choice('blog.comments', count($article->comments))}}
            </h3>
            @set($commentsAll, $article->comments->groupBy('parentId'))
            <ol class="commentlist group">
                @foreach($commentsAll  as $k => $comments)
                    @if($k != 0)
                        @break
                    @endif
                    @include(env('THEME'). '.blog_article_comment', ['items' => $comments])
                @endforeach
            </ol>
